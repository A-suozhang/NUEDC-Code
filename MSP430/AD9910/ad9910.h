...
class AD9910
{
    ...
    unsigned char cfr1[4] = {0x00, 0x40, 0x00, 0x02};
    unsigned char cfr2[4] = {0x01, 0x00, 0x08, 0x20};
    unsigned char cfr3[4] = {0x05, 0x08, 0x41, 0x32};
    unsigned char DAC_config[4] = {0x00,0x00,0x00,0xFF};
    unsigned char profile0[8] = {0x3F, 0x3F, 0x00, 0x00, 0x25, 0x09, 0x7b, 0x42};
    // ----Constructor----
    AD9910(int cs , int rst , int update , int sdio , int sclk )
    {
        _cs  = cs ;
        _rst  = rst ;
        _update  = update ;
    ...
    void SPI_Write_Reg(uchar addr, uchar bytes[], uchar num_bytes)
    {
        SPI.beginTransaction(SPISettings(CLOCKSPEED, MSBFIRST, SPI_MODE0)); // 开始SPI传输,其中CLOCKSPEED是SCLK传输,选用MODE0(经测试除了MODE1之外其他MODE都能操作)
    ...
    void set_freq(double freq, uchar profile = 0)
    {
        if (profile > 7){
            return;
        }
    ...
    void set_Amp(double amp, uchar profile = 0)
    {
        unsigned long temp;
